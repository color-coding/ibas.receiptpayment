...
import { PaymentViewApp } from "./PaymentViewApp";
import { PaymentEditApp } from "./PaymentEditApp";

/** 列表应用-付款 */
export class PaymentListApp extends ibas.BOListApplication<IPaymentListView, bo.Payment> {

    /** 应用标识 */
    static APPLICATION_ID: string = "e9fd403f-988b-4b35-978f-ee05c4aabfb0";
    /** 应用名称 */
    static APPLICATION_NAME: string = "receiptpayment_app_payment_list";
    /** 业务对象编码 */
    static BUSINESS_OBJECT_CODE: string = bo.Payment.BUSINESS_OBJECT_CODE;
    /** 构造函数 */
    constructor() {
        super();
        this.id = PaymentListApp.APPLICATION_ID;
        this.name = PaymentListApp.APPLICATION_NAME;
        this.boCode = PaymentListApp.BUSINESS_OBJECT_CODE;
    ...
    protected viewData(data: bo.Payment): void {
        ...
        let app: PaymentViewApp = new PaymentViewApp();
        app.navigation = this.navigation;
        app.viewShower = this.viewShower;
        app.run(data);
    ...
    protected deleteData(data: bo.Payment | bo.Payment[]): void {
        ...
        let beDeleteds: ibas.ArrayList<bo.Payment> = new ibas.ArrayList<bo.Payment>();
        ...
        this.messages({
            ...
            onCompleted(action: ibas.emMessageAction): void {
                if (action === ibas.emMessageAction.YES) {
                    try {
                        let boRepository: BORepositoryReceiptPayment = new BORepositoryReceiptPayment();
                        let saveMethod: Function = function (beSaved: bo.Payment): void {
...
/** 视图-付款 */
export interface IPaymentListView extends ibas.IBOListView {
    /** 编辑数据事件，参数：编辑对象 */
