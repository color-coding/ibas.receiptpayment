...
public class ReceiptItem extends BusinessObject<ReceiptItem> implements IReceiptItem, IBusinessLogicsHost,
	 ...
	 * 原始类型 属性
	 */
	@DbField(name = "OrgnlType", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 原始标识 属性
	 */
	@DbField(name = "OrgnlEntry", type = DataType.NUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 原始行号 属性
	 */
	@DbField(name = "OrgnlLine", type = DataType.NUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 终端客户 属性
	 */
	@DbField(name = "Consumer", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME)
	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new IReceiptPaidTotalContract() {
			...
			public BigDecimal getAmount() {
				if (MyConfiguration.applyVariables(SalesCreditNote.BUSINESS_OBJECT_CODE)
						.equalsIgnoreCase(this.getBaseDocumentType())) {
					return ReceiptItem.this.getAmount().abs();
				}
